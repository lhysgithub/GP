...
研究成果，学习成果，往往都是冲下来的，所以，不要吝惜你的时间，但也要珍惜你的时间。努力学习，少玩儿游戏，以后会有你玩游戏的时间的，会有的。
...
1. 完成logits和preds代表含义的研究
2. 考虑目标分类不在前k中分类时如何完成定向错分
3. 原程序中使用的是logit，现在使用置信度来做是否能做？
4. 对于攻击算法的，需要在看看循环部分的核心代码
5. 读写文件太费时间，看能否直接传入API?
...
1. 针对pi-nes循环部分，我们探究其每次梯度下降的正确性。
2. 测试利用noise和grade的值来

今日完成：

1. 解决图片

本实验策略，原分类附近梯度可算，所以我们能减少原特征。

简直，天要绝我，今天发现，腾讯的API居然有防御功能，用了梯度消失。

##### 2018/03/14

目前存在的问题：

1. 腾讯物体识别梯度消失，利用置信度很难估计梯度
2. 腾讯图像标签识别得加后很难找到想要的分类上升，并且，没有分类种类数量。
3. 阿里云查询数量有限，并且，阿里这边没有明确分类种类。
