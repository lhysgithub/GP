...
1. 成功加入全局最优解这个变量
   ...
   ​	本算法只是用自然进化算法估计梯度值，然后使用梯度下降的方法，寻找最优解，这个算法还是基于数学的逻辑，使用梯度作为核心搜索方向，这使得搜索过程十分抖动。再由于要进行网络查询，迭代次数不能过高，而原算法所需查询次数过度，耗时过长。解决方案是加入贪心的策略使得，参考进化算法搜索最优解的策略，而非单纯利用自然进化估计梯度值，而真切的使用自然进化算法搜索最优解。

##### 2018/03/15 

​单独加入全局最优解并不能改变其本质，现在测试加入动量。

如果加入动量也不行，那就考虑利用进化算法修正迭代。

考虑，每当adv更新的时候也要更新bestadv

测试：增大batchsize并增加更新bestadv

存在问题：

1. bestloss可能更新有问题，考虑loss和logit不单调。
2. 考虑变换高斯模型参数。

目前任务：

1. 寻找更加优质的源目图片（测试是否真的梯度消失）
2. 更改算法，改成非基于梯度的攻击算法。（进化算法）
...
1. 完成汇报PPT5页，周日前完成。（算法，优化，实验（对比优化内容），结论，方向）

2. 完成并行查询腾讯API

3. 完成先错分在针对错分/由top5->top1的错分。
...
#####2018/03/19
1. 注意有的c不发音，scene，scent等，也有的发k的音，scratch 


##### 2018/03/21

1. 投影梯度可以参考，但是，0.5的值实在是太低了，令人发指的噪声量。。。
2. 主要问题，没有考虑投影之后与原图的最小扰动
3. 之所以会使得原始图片越来越像目标图片，是因为该算法到最后会减小搜索域，然而搜索域的中心是目标图片，而非原始图片。。。（我只能说原作者SB）
...
1. 问题：梯度算出来没大用。
   ...
   - 梯度还不够准，看一下是否是样本选取数量过少
   - 样本选取选取方式的问题
   - 搜索成组的优秀对抗图片、
   - 完成先错分在针对错分/由top5->top1的错分。
...
1. 差分进化主体算法完成（缺失生成新子代的操作，可能是循环层数太多的问题；差debug）
...
​	每日，都想着要学英语；每日，都会忘记学英语；究竟是时间不够，还是你对英语的抵触？这个问题只有你自己清楚，别再托了，马上就4月份了。时间不会等你，只有结果会等你。你想要当自由自在的飞鸟，还是受困牢笼的困兽，这由你决定。加油，洪毅。
...
​	今天已经4月9日了，距离毕设中期已经很近了，距离托福考试也十分的接近，或许，你现在还做不到全天投入到学习中，但是目前，你只要需要拿出一半以上的时间来完成这项工作。
