...
class TencentAPIMsg(object):
    ...
    def __get_imgfile_base64str__(self, image):
        ...
        #     outputBuffer = BytesIO(bytes(np.array(image,dtype='uint8')))
...
def ExecTecentAPI(*arg ,**kwds):
    ...
    for each in resp.json()['data']['object_list']:
        text = text + str(each['label_id']) + ' ' + str(each['label_confd']) + '\n'

    # print (text) 确实能返回正确的分类信息
    return  text

#改成你自己腾讯APPID及APPKEY
APPID = '1106687579'
APPKEY = 'LcnJV9dLhaGW1IJc'

# 大概完成model部分吃完饭回来测试
def Predict(NewImages,batch_size):
    # 将(batch_size,299,299,3)NewImages传给API
    ImageNP = np.array(NewImages)
    NewImages = np.ndarray.tolist(NewImages)
    confidence = [1000*[0.0]]*len(NewImages)
    prediction = [0]*len(NewImages)

    # temp = NewImages.ndim()
    for i in range(len(NewImages)):
        if os.path.exists('./request.jpg'):
            os.remove('./request.jpg')
        scipy.misc.imsave(os.path.join('./', 'request.jpg'), NewImages[i])
        print("request ",i)
        # temp = Image.fromarray(np.array(ImageNP[i]*255,dtype='uint8'))
        rest = ExecTecentAPI(image= './request.jpg')
        # rest = ExecTecentAPI(image= temp )
        # print(rest)
        rest = rest.split()
        for j in range(5):
            confidence[i][int(rest[j*2])] = float(rest[j*2+1])
            if j == 1:
                prediction[i] = int(rest[j*2])
        temp = (1 - sum(confidence[i]))/995
        for j in range(1000):
            if confidence[i][j] == 0.0:
                confidence[i][j] = temp
